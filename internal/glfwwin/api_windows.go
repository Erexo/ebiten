...
var (
	...
	procIsWindows7OrGreater       = kernel32.NewProc("IsWindows7OrGreater")
	procIsWindows8OrGreater       = kernel32.NewProc("IsWindows8OrGreater")
	...
	procIsWindowsXPOrGreater      = kernel32.NewProc("IsWindowsXPOrGreater")
...
func _IsWindows7OrGreater() bool {
	r, _, _ := procIsWindows7OrGreater.Call()
	return int32(r) != 0
}

func _IsWindows8OrGreater() bool {
	r, _, _ := procIsWindows8OrGreater.Call()
	return int32(r) != 0
}
...
func _IsWindowsXPOrGreater() bool {
	r, _, _ := procIsWindowsXPOrGreater.Call()
	return int32(r) != 0
}
...
func _SendMessage(hWnd windows.HWND, msg uint32, wParam _WPARAM, lParam _LPARAM) _LRESULT {
	r, _, _ := procSendMessage.Call(uintptr(hWnd), uintptr(msg), uintptr(wParam), uintptr(lParam))
	return _LRESULT(r)
}
...
func _SetFocus(hWnd windows.HWND) error {
	r, _, e := procSetFocus.Call(uintptr(hWnd))
	if int32(r) != 0 {
		return fmt.Errorf("glfwwin: SetFocus failed: %w", e)
	}
	return nil
}
