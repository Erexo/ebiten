...
func (w *Window) SetAspectRatio(numer, denom int) error {
	...
	if numer != DontCare && denom != DontCare {
		if numer <= 0 || denom <= 0 {
			return fmt.Errorf("glfwwin: invalid window aspect ratio %d:%d: %w", InvalidValue)
		}
	}
...
func (w *Window) ShowWindow() error {
	if !_glfw.initialized {
		return NotInitialized
	}
	if w.monitor != nil {
...
func (w *Window) GetAttrib(attrib int) (int, error) {
	if !_glfw.initialized {
		return 0, NotInitialized
	}
...
func (w *Window) SetAttrib(attrib int, value int) error {
	if !_glfw.initialized {
		return NotInitialized
	}
