...
func windowProc(hWnd windows.HWND, uMsg uint32, wParam _WPARAM, lParam _LPARAM) _LRESULT {
		...
		if wParam >= 0xd800 && wParam <= 0xdbff {
			...
			if wParam >= 0xdc00 && wParam <= 0xdfff {
				if window.win32.highSurrogate != 0 {
					codepoint += (uint32(window.win32.highSurrogate) - 0xd800) << 10
					codepoint += (uint32(wParam) & 0xffff) - 0xdc00
					codepoint += 0x10000
				}
			} else {
				codepoint = uint32(wParam) & 0xffff
			}
	...
	case _WM_UNICHAR:
		...
		window.inputChar(uint32(wParam), getKeyMods(), true)
		return 0
...
func (w *Window) platformSetWindowIcon(images []*Image) error {
	...
	_SendMessage(w.win32.handle, _WM_SETICON, _ICON_BIG, _LPARAM(bigIcon))
	_SendMessage(w.win32.handle, _WM_SETICON, _ICON_SMALL, _LPARAM(smallIcon))
...
func (w *Window) platformFocusWindow() error {
	...
	if err := _SetFocus(w.win32.handle); err != nil {
		return err
	}
	return nil
}
...
func platformSetClipboardString(str string) {
...
func (w *Window) glfwGetWin32Window() windows.HWND {
	if !_glfw.initialized {
		panic(NotInitialized)
	}
	return w.win32.handle
}
